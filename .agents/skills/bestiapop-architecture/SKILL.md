...
| Servicio | Rol |
|----------|-----|
| `MusicService` | `MediaLibraryService` + ExoPlayer (`WAKE_MODE_LOCAL` / `WAKE_MODE_NETWORK` dinámico + audio offload) + WakeLock de servicio dedicado durante reproducción/transiciones; única notif Media3 `RefreshingMediaNotificationProvider` → `DefaultMediaNotificationProvider` en `playback_channel`/1001, 10 min de gracia y FGS `mediaPlayback` con retención de foreground activa mientras `playWhenReady && mediaItemCount > 0`; `onUpdateNotificationAsync` y refresh de artwork conservan Remote `STATE_IDLE`; `onTaskRemoved` usa política propia. `MediaButtonReceiver` + resumption restauran System UI/Bluetooth; callback separado sirve browse Auto/AVRCP y concede solo transporte a launchers/widgets no confiables. |
