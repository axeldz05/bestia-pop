...
| Persistencia | Room (`bestiapop_music_db`, v15) |
...
| Servicio | Rol |
|----------|-----|
| `MusicService` | `MediaLibraryService` + ExoPlayer (`WAKE_MODE_NONE` local / `WAKE_MODE_NETWORK` remoto) con `DefaultRenderersFactory` (fallback habilitado y selector de códecs que prioriza decodificadores de audio software estándar sobre códecs vendor propensos a fallos de buffer) + WakeLock transitorio en buffering/transiciones; única notif Media3 `RefreshingMediaNotificationProvider` → `DefaultMediaNotificationProvider`, canal `playback_channel`/1001, 10 min de gracia y FGS `mediaPlayback` mientras `playWhenReady && mediaItemCount > 0`; rescate directo `ServiceCompat.startForeground` en `handleForegroundServiceStartDenied` si Android 14+ bloquea `startForegroundService` en segundo plano; `onStartCommand` retorna `START_NOT_STICKY` para evitar resurrecciones zombies; notif 1002 solo si `blocksBackgroundPlayback` se confirma (`maybeNotifyBackgroundRestriction`) y se cancela al levantar la restricción. `onUpdateNotificationAsync` y refresh de artwork conservan Remote `STATE_IDLE`; `onTaskRemoved` detiene la reproducción y limpia el servicio/notificaciones de inmediato cuando el usuario descarta la app del gestor de tareas (`PlaybackServiceLifetimePolicy.shouldStopAfterTaskRemoved` = `true`). `MediaButtonReceiver` + resumption restauran System UI/Bluetooth; callback separado sirve browse Auto/AVRCP y concede solo transporte a launchers/widgets no confiables. |
...
| `WebServerService` | Servidor Ktor local para sync/upload por WiFi. FGS `dataSync` (no `mediaPlayback`) vía `ServiceCompat.startForeground`; `onTimeout` Android 15+ marca transferencias activas como error y cierra limpiamente; `onTaskRemoved` detiene el servidor y remueve el foreground si el usuario cierra la app. Subidas: validación de `Host` + tope `MAX_UPLOAD_BYTES` también sobre el stream |

## Base de datos

Entidades: filas de app en `data.model` — `Song` (`songs`), `AlbumOverride` (`album_overrides`), `SongPlayStat` (`song_play_stats`). El resto sigue en `data/db`: `PlaylistEntity`, `PlaylistSongCrossRef`, `PlaylistPendingTrackEntity`, `PendingListenEntity`, `ArtistEntity`, `GenreEntity`, `SongArtistCrossRef`, `SongGenreCrossRef`, `AlbumArtistCrossRef`, `AlbumGenreCrossRef`.
