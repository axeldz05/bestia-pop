...
**Invariante bytes (`downloadAndSaveOnlineTrack`):** solo se publica el archivo si la descarga terminó completa. Se exige `downloadSuccess` **y** el total contra `Content-Length` (EOF corto = truncado, no éxito); se concatena solo con `206`; el stream va en `use`. Googlevideo manda `Range` cerrado **por chunks** (`GoogleVideoRange.nextChunkRange`, nunca `bytes=0-{clen-1}` en un GET: el CDN 403 al archivo entero). `YouTubeExtractor.AUDIO_CLIENTS` no usa ANDROID_VR 1.65.10 (403 tras ~1MB desde 2026.08.17): TVHTML5 5.x, VISIONOS, ANDROID, ANDROID_MUSIC, TVHTML5 7.x. Ante `403`/`410` se re-extrae; otros hosts solo Range al reanudar (`bytes=N-`). Al fallar se borra el parcial. El loop L1 `copyTransferToFile` se comparte con APK update; Range/retry/validación/publicación siguen en cada dominio.
...
| Filtrado/orden | `GetLibrarySongsUseCase.execute(songs, query, sortOption, sortDirection, haystackById?)` — needle vía `TrackMatchKeys.normalize`; haystack (`searchHaystack` + runs de script) solo si query no-vacía (`LibraryProjectionState`); query no-vacía debounce `LIBRARY_SEARCH_DEBOUNCE_MS`; punctuation-only → no match |
| Vista plana vs grupos álbum | `LibraryViewMode.FLAT` / `ALBUM_GROUPS` → `setLibraryViewMode` / `toggleLibraryViewMode`; solo UI cuando chip = `SONGS`; `LibraryListModel` (`songsVisual` + `LibraryAlbumSegment` start/count/`songIds`; LazyColumn `items(count)` / `keyAt` / `itemAt`); `buildLibraryListModel` / `buildListModel` / `projectCatalog` (bloques = `extractAlbums` por `songsByAlbumBucket`; cada canción en **un** grupo; key = `Album.groupingKey` / `LibraryAlbumSegment.groupingKey`); `ALBUM_GROUPS` no hace sort global por título; `itemAt` usa `songIds` precacheados + `sortHint`; within-album `compareSongsWithinAlbum` / `sortSongsWithinAlbum` / `songsFromListItems` / `songsForAlbum` / `songsInOrder` |
...
| Multi-select | La selección **se mantiene** al buscar: `LibraryScreen` cachea `selectedSongsById` al tildar (no relee `rawSongs`). La fila de chips se oculta mientras hay selección: con chips de agregados, «Seleccionar todo» tomaba toda la biblioteca |
| Colapsar álbum / todos | `collapsedAlbumNames` + toggle por header (`onToggleCollapseAlbum`); `LibraryListModel.collapsed` / `filterCollapsedAlbumSongs` oculta filas bajo el header (`matchesCollapsed` por `albumName`/`groupingKey`); expandir/colapsar todo usa `LibraryListModel.albumNames` (`LibraryScreen`; vista grupos; no persistido) |
| Derivados | `LibraryProjectionState.projectCatalog` espera `prefsReady` → `{songs,albums,list}` Eagerly + `catalogLoaded` (spinner vs vacío tras proyectar); `extractArtists` / `extractGenres` / `recentSongs` WhileSubscribed (chip o NP). `sameLibraryCatalog` ignora `lastPlayedAt`; Recientes combina `rawSongs` + `songPlayStatsFlow`. Play-all/tap Canciones = `songList.songsVisual` (`playCurrentLibraryBrowse`). `allSongsFlow` es identity-slim compartido (`shareIn`, `lastPlayedAt=0`); NP/editar letra hidratan con `getSongById`. Nested restore: `pruneRestoredLibraryStack` tras `catalogLoaded` vía `LibraryStackLookups.fromSongs` |
| Énfasis agregados | Álbumes: `AlbumHeader.sortHint` vía `formatSortRelevantInfo`; artistas/géneros: subtítulo con mismo helper |
| RECENT | Chip label **Recientes**; canciones con stamp en `song_play_stats` DESC; `touchSongLastPlayed` → `MusicDao.updateLastPlayedAt` (tabla stats, no `songs`); stamp `setCurrentItem` → `maybeTouchSongLastPlayed` (dedupe por songId / mismo mediaId; sin enhance extra en re-set); `emphasizeLastPlayed`; sección orden del sheet deshabilitada en RECENT (también con nested) |
| Blobs / chips configurables | `LibraryBlobsSettings` + `LibraryBlobConfig` → `setLibraryBlobsSettings` en `LibraryPreferencesRepository` (persistido vía `LibraryUiPreferencesCodec.encodeBlobsSettings`/`decodeBlobsSettings`); UI en Ajustes → Biblioteca (`LibrarySettingsScreen`) con reordenamiento interactivo vertical (`rememberVerticalReorderDrag` + drag handle `Icons.Default.DragHandle`), toggle on/off (mínimo 1 activo obligatorio) y badge «Principal» en el primer blob activo; `LibraryFilterChipRow` renderiza `libraryBlobsSettings.enabledFilters` en el orden personalizado y `LibraryScreen` redirige automáticamente a `primaryFilter` si el filtro activo es deshabilitado. |
| Fast scroll Niagara | Scroll vertical rápido adaptativo con tick háptico, burbuja flotante y píldora indicadora de posición (`FastScrollScrubber` + `FastScrollSections` + `NaturalTextOrder`). Orden natural: A-Z con elementos reales, seguido de una sección de números (`0-9`) y finalmente una de símbolos especiales (`*`). Romanización automática de títulos bilingües entre paréntesis/corchetes (ej. `夜鷹 (Yodaka)` -> `Y`) o transliteración ICU a alfabeto latino/español. Con vista de grupos de álbum activa, indexa exclusivamente los album headers sin mezclar letras de canciones intermedias. Indicador visual de posición en el riel (letra activa destacada + thumb deslizable). Umbral mínimo ≥12 ítems. Espacio dedicado en el margen activo (`FastScrollContainer` + `FastScrollDefaults.dedicatedStartGutterWidth` / `dedicatedEndGutterWidth`) para no superponer portadas ni checkboxes en el lado izquierdo ni botones de acción (menú ⋮, play/shuffle) en el lado derecho. Configurable en Ajustes → Biblioteca (`LibrarySettingsScreen`: activar/desactivar y lado derecho/izquierdo para zurdos). |
| Preservación de scroll | `LazyListState` hoisted en `LibraryScreen` mediante `LibraryBrowseListStates` (`rememberLibraryBrowseListStates`) con `rememberSaveable` para cada sección (`songs`, `albums`, `artists`, `genres`, `playlists`, `recent`) y `nestedListState` para el drill-down, reteniendo la posición exacta al alternar tabs y al volver atrás. Detalle de playlist usa `rememberSaveable(playlist.id)` en `PlaylistsScreen`. |

UI: `LibraryScreen` (`NestedLibraryBrowse` para detalle álbum/artista/género), `LibraryFilterChipRow`, `LibraryBrowseSortSheet`, `LibrarySongList` (`FastScrollContainer`), `LibraryAlbumBrowseList` (`AlbumHeader` + `FastScrollContainer`), `LibraryAggregateListItem` + `LibraryArtistList` / `LibraryGenreList` (`FastScrollContainer`), `LibrarySettingsScreen`.
Estado: `ui/state/LibraryProjectionState.kt`, `UiNavigationState.kt`, `LibraryBrowseFilter.kt`, `LibraryListItem.kt` (`LibraryListModel` / `LibraryAlbumSegment`) y `LibraryUiState.kt` (solo `LibraryViewMode`). Prefs: `LibraryDisplaySettings` (`sortOptionName` / `sortDirectionName`), `LibraryBlobsSettings` (`items: List<LibraryBlobConfig>`), `FastScrollSettings` (`FastScrollSide.RIGHT` / `LEFT`) + `UiNavSnapshot.browseFilterName` / `LibraryUiPreferencesCodec` (legacy `library_tab` 0/1/2 → SONGS/ALBUMS/ARTISTS).
...
| **Álbum menú** | Header de grupos y browse (`AlbumHeader` ⋮) → Editar / Cambiar portada vía `AlbumEditCoverMenuItems`; detalle de álbum también tiene IconButton Edit | `LibrarySongList` / `LibraryAlbumBrowseList` / `AlbumEditCoverMenuItems` / `LibraryScreen` |
| **Álbum merge** | Renombrar a un álbum existente → `ConfirmMergeAlbumsDialog`; al confirmar, canciones de A adoptan metadata de B. Match con `albumIdentityKey` (case/tildes, deluxe/EP, CJK, `…`) vía `requestSaveAlbumMetadata`. `mergeAlbumInto` / portada pliegan `libraryAlbumKeysInBucket` | `requestSaveAlbumMetadata` / `confirmPendingAlbumMerge` / `findAlbumMergeTarget` / `AlbumNames.kt` |
...
Headers de grupo: `LibraryListItem.AlbumHeader` lleva `albumName` (clave), `displayName` (override), `songIds` y `sortHint` — `AlbumHeader(title = …)` muestra el display name, así que Canciones y Álbumes no divergen tras renombrar sin propagar. Play/select del header resuelve ids con `songsInOrder`.

Herencia visual en lista: `GetLibrarySongsUseCase.projectCatalog` / `buildListModel` unifica artwork faltante al armar el índice (`inheritedArtworkBySongId` → `LibraryListItem.SongRow.artworkUri` en `itemAt`; **omite álbumes genéricos**: «Unknown Album» es el literal de todas las canciones sin álbum, así que heredar ahí estampaba una portada sobre temas no relacionados). El pool `CatalogProjection.songs` no copia `Song`. `execute` solo filtra/ordena. `extractAlbums(songs, overrides, sortOption, sortDirection)` agrupa por `albumGroupingKey` (no el string crudo), aplica `AlbumOverride` y ordena agregados.

## 5. Playlists locales

CRUD + membresía vía `IMusicRepository`:
`createPlaylist`, `updatePlaylist`, `deletePlaylist`, `addSongToPlaylist`, `addSongsToPlaylist`, `removeSongFromPlaylist`.
- **Canciones duplicadas y orden:** `PlaylistSongCrossRef` (`playlist_song_cross_ref`) utiliza clave primaria autonumérica `id: Long` con índices en `playlistId` y `songId`, permitiendo múltiples instancias de una misma canción en una playlist. Las canciones se ordenan por `refs.position ASC, refs.id ASC`. Al eliminar (`removeSongFromPlaylist`), se elimina solo la primera ocurrencia encontrada.
- **Reordenar canciones:** En el detalle de la playlist (`PlaylistDetailScreen`), el usuario puede activar el modo "Mover canciones" ("Mover" / "Listo"). En este modo, el menú de opciones (⋮) de cada canción se reemplaza por un deslizador/drag handle (☰, `Icons.Default.DragHandle`), permitiendo arrastrar individualmente la canción verticalmente para cambiar su orden en la lista con feedback visual inmediato. Al soltar, la posición se persiste en Room mediante `MusicDao.reorderPlaylistSongs` actualizando las posiciones correlativas en `playlist_song_cross_ref`.
- **Portadas automáticas:** Si el usuario no asigna una portada personalizada a la playlist (`coverUri == null`), `getAllPlaylistsFlow` y `getPlaylistDetailsFlow` resuelven dinámicamente (`COALESCE`) la carátula de la primera canción de la playlist que tenga portada disponible (`refs.position ASC, refs.id ASC`). Al crear playlist desde canciones seleccionadas, `AddToPlaylistDialog` precarga además la portada de la primera canción seleccionada como sugerencia en el diálogo.
- **Creación y adición:** Diálogos reutilizables `PlaylistFormDialog` y `AddToPlaylistDialog` permiten tanto añadir/crear como "Crear/Añadir y entrar" (`onSaveAndOpen` / `onAddAndOpen`) para navegar de inmediato al detalle de la playlist si el usuario lo desea. Admite selección múltiple de canciones.
...
- **Fecha de adición real del dispositivo:** `scanMediaStore` lee `DATE_ADDED` / `DATE_MODIFIED` de MediaStore (o fecha del archivo); el escaneo de carpetas SAF (`scanFolderUri`) y de la carpeta app-managed (`indexSourcesParallel`) toma `file.lastModified()`, asignando a `Song.dateAdded` la fecha real en el dispositivo en lugar de la hora de escaneo. Migración one-shot `migrateDateAddedFromDevice()` (`device_date_added_migrated`) actualiza en segundo plano las canciones ya indexadas previamente.
- **Pases de fondo con memoria de intentos:** el enriquecido de portadas/letras (`enhanceSongMetadataAndLyricsBatch`) y las fotos de artista esperan la primera lista no vacía + idle (`FIRST_LIBRARY_IDLE_MS`), colectan `rawSongs` (no `libraryProjection.songs`) y marcan lo ya intentado (`metadataEnhanceAttempted` / `artistPhotoAttempted`). `migrateCanonicalAudioUris` / `migrateDateAddedFromDevice` son one-shot y corren **después** de `catalogLoaded`; `migrateLegacyYouTubeMusicSongs` espera `awaitFirstLibraryIdle()` (red). Writes en una `@Transaction`.
...
- Import SAF (`scanFolderUri`) enumera el árbol una sola vez (conserva total/progreso exactos). Si el tree URI mapea a un directorio real (`SongPathNormalizer.toAbsolutePath` / `/tree/` + `/document/`) y `File.listFiles` responde, camina el filesystem; si no, DocumentFile. El banner `LibraryJobKind.IMPORT` se publica **antes** del walk (`importFolder` → «Buscando archivos…», `total=0` indeterminado + conteo al encontrar) y recién después el ticker done/total al indexar. Tags+artwork en paralelo (`SCAN_FILE_PARALLEL=4`); `AlbumArtworkCache` reusa portada por artista+álbum (salta `embeddedPicture` en el resto del álbum). Guarda `AudioPersistRef` (abs si el document id mapea a filesystem). Reproducción: `playableUri` (SAF viejo → `file://`). Fallo local → toast «No se pudo reproducir».
...
- Tags Unknown / rip numérico: `AudioFileMetadata.fromPath` lee jaudiotagger primero (mp3/m4a/flac/ogg) y `MediaMetadataRetriever` de fallback; Unknown/Music solo si **ambos** fallan. `applyFilenameHints` + `parseFilenameMetadataHints` / `resolveWeakIdentityHints` / `tidyFilenamePhrase` / `identifySearchTexts` (`IdentifyQueryVariants`: runs por script Unicode, cabeza latina vs cola pinyin, sufijo OST en rips `1-07.`); al `proposeSongIdentity` persiste limpia soft de **basura** (`01`→Unknown Artist, `- Title`→Title, trackNumber, path) **sin** pisar un título ID3 real. Filename hints siguen armando la **consulta**. `Artist_Title` solo parte en un `_` (`Radiohead_Creep`); varios `_` quedan frase buscable (`The_Doors_Roadhouse_Blues`) y `splitUsingKnownArtists` parte contra artistas ya en biblioteca. Huecos del sanitizer (`__` CJK/tildes, `It_s`→`It's`, `16-44.1 kHz`) se colapsan. Identify: ListenBrainz (token) → MusicBrainz → Deezer/iTunes → YouTube (`fetchIdentifyCatalogTracks`); si no HIGH, variantes + `searchIdentifyFallbacks` (iTunes US/JP/TW + MusicBrainz + YouTube). Títulos bilingües al aplicar: `preferBilingualTitle` (`夜鷹 (Yodaka)`).
- Identificar online (manual, multi-select, ⋮ o WiFi): lote **process-scoped** (`ProcessIdentifyRuntime.submit`; no `viewModelScope`). UIDT API 34+ / FGS `dataSync` API 26–33 mientras hay remaining. Fase 1 lookup + score (`IdentifyRanking`); tags actuales de la canción son **fuente predominante** (`sourceArtist`/`sourceTitle`/`sourceAlbum`); auto-aplica solo **HIGH** y **no pisa el título** salvo script complementario (`preferBilingualTitle` / `shouldApplyBilingualTitle`) o `fillGapsOnly` + título basura. En revisión el usuario sí puede aceptar otro. Conflicto grave (artista/álbum/título distinto, versión, YouTube), título genérico (`isGenericIdentifyTitle`: Castle/Black Hole/…) o título igual a artista/álbum (`titleCollidesWithArtistOrAlbum`) → nunca HIGH (busca `catalogSearchText` artista+álbum y rankea por duración). Duración lejana **penaliza**; un único hit ±2s con overlap de título puede subir a HIGH aunque el artista sea Unknown. Score también compara por script (`scriptRunSimilarity`) y artista+título juntos (`fieldSimilarity` / `compactSimilarity`) para no penalizar el split `The`/`Doors Roadhouse Blues` ni un `sourceTitle` concatenado (`Clever Girl Elm` vs `Elm`) cuando el catálogo tiene el artista+título correctos. Si hay token LB, `lookupRecordingMetadata` corre **antes** que Deezer/iTunes (títulos genéricos no lo bloquean si hay duración o tags de archivo) → `fetchRecordingMetadata` → `toListenBrainzCatalogTrack` (`IdentifyProposal.usedListenBrainz`); después MusicBrainz y el catálogo. MEDIUM/LOW/NONE se **bufferizan** y se anexan a cola persistida `IdentifyReviewStore` en ráfagas (`PERSIST_EVERY=8` + flush al terminar/interrupt; DataStore; cold start hidrata, **no** abre overlay). Remaining IDs en `IdentifyWorkStore` (mismo coalesce). Identify remaining resume en `onAppForeground` siempre; descargas siguen salvo `REASON_USER_REQUESTED`. UI `IdentifyReviewScreen`: overview por álbum sugerido (`clusterIdentifyAlbumGroups`, solo MEDIUM + álbum no genérico, size ≥ 2; `albumGroups` lazy) o cola canción a canción (Usar / Omitir / Buscar otro + filtros artista·álbum·año / Mostrar más candidatos / Aplicar automático a restantes = solo MEDIUM con suggested / Omitir todas / Aplicar grupo). Header muestra chips editables de `IdentifyApplyFields` (`IdentifyApplyField` / `setIdentifyReviewApplyFields`; persistidos con la cola). Cada candidato lista diffs (`identifyApplyChanges` / `formatIdentifyApplyChanges`) solo de campos tildados que cambiarían. “Buscar otro” siembra **título** en el draft y artista/álbum/año en filtros (`identifySearchDraft` / `identifySearchFilterArtist`); no concatena artista+título. «Tu archivo» muestra año/pista y aviso si el título colisiona. Cerrar oculta overlay y **conserva** la cola (`pendingCount`); banner biblioteca + botón WiFi + notif retoman. Re-identificar lote/WiFi **omite** songIds ya pendientes (`pendingSongIds` cacheado; cero red); ⋮ sobre pending abre ese ítem. Un segundo pedido **agrega** IDs al lote en curso. Overlay ya abierto: anexar propuestas (`mergeIncomingReviewItems`) **no** reinicia buscador, filtros, selección ni candidatos del ítem actual; `showIdentifyReview` no-op si ya visible. Progreso `libraryJobProgress` (merge runtime IDENTIFY + import/tag-write local) + toast resumen si hay UI; overlay al completar solo si `showReview` y UI attached. Telemetría lote (sin PII): `CrashReporter.setKey`/`log` keys `identify_high`/`identify_medium`/`identify_low`/`identify_none`/`identify_skipped`/`identify_lb_hits`.
- «Título distinto» se mide contra el tag propio (`sourceTitle` vs candidato) **aunque** `identityBagMatch` (un query de filename `Creep` no auto-aplica sobre ID3 `Radiohead`). `titleSim` compara la consulta.
- `applySongIdentity` **conserva la duración local** cuando es > 0: `mergePreferring` prioriza el receptor (el candidato del catálogo), así que una remasterización o el default de 180000ms de iTunes reescribían el largo real. Álbum/artista: `pickPersistedAlbumName` / `pickPersistedArtistName` reusan el string ya en biblioteca (eureka/Eureka, deluxe/EP, Gling-Gló); sesión live (Audiotree…) no pisa un álbum de estudio.
- Álbum genérico (`IdentifyRanking.isGenericAlbum`: `YouTube`, `YouTube Music`, `Unknown Album`, `Single`, `Álbum`/`Album`) **sí** entra a identify; no se omite como ya identificado. Provider YouTube y versiones extra (live / letra / remix / cover…) nunca son HIGH → review. `cleanIdentityTitle(raw, artist)` limpia nombres de artista (prefijos/sufijos), numeración de pista inicial (`01. `, `01 - `, `1-03. `), comillas y expresiones cosméticas en español e inglés (`(Official Video)`, `(Video Oficial)`, `(Audio Oficial)`, `[4K]`, `[1080p]`, `(Remastered)`, `(Album Version)`, etc.) preservando versiones musicales reales (`Live`, `Acoustic`, `Remix`). Pista/disco se resuelve con tracklist del álbum, MusicBrainz, formato multidisco (`parseCdTrackNumber`) y fusión Deezer+iTunes.
...
- Import SAF, primer arranque (`resyncAppManagedMusic` + `scanMediaStore`) y WiFi disparan `identifyImportedGaps`: solo huecos esenciales (`IdentifyGaps.needsGapIdentify` / `gapApplyFields`: artista placeholder, álbum genérico, título basura, portada no usable). Año y nº de pista **no** disparan ni se auto-aplican. `fillGapsOnly=true`, `force=false`, `showReview=false`. HIGH aplica **solo** esos campos (título auto únicamente si es basura). Completos no van a catálogo. Sin red el lote pausa (`interrupted`, no NONE) y retoma en `onAppForeground`.
- Álbumes **ya conocidos** (biblioteca no genérica) alimentan match 1:1 por título/duración/pista (`IdentifyKnownAlbums` / `assignUniqueKnownAlbumMatches`). En lotes, `ProcessIdentifyRuntime` ejecuta 100% en segundo plano (`Dispatchers.IO`), ordena la cola por carpeta y álbum para procesar tracks contiguamente y maximizar el *fan-out* oficial (`fanOutKnownAlbum`) sin bloquear el hilo principal ni la biblioteca. El diálogo `IdentifySetupDialog` permite elegir entre «Solo datos faltantes» (`songHasGapsForFields`) o «Todas las seleccionadas». En `proposeSongIdentity` un hit **global único** es HIGH sin catálogo. Tras Usar/HIGH, fan-out **scoped** al tracklist Room+Deezer/iTunes (`loadKnownAlbumTracks`) aplica siblings únicos de la cola; ambiguos quedan. En revisión el fan-out corre **después** de publicar leftover. Al hidratar revisión, hits globales se promocionan a MEDIUM para el overview (`attachKnownAlbumMatches`).

| Acción | API |
|--------|-----|
| Scan MediaStore | `scanMediaStore(onProgress?): List<Song>` (skip BestiaPop + path/matchKey conocidos; VM `identifyImportedGaps`) |
| Reindex app music | `resyncAppManagedMusic(onProgress?): List<Song>` → `Music/BestiaPop` filesystem walk; VM `ensureInitialLibraryImport` (1ª vez) + `identifyImportedGaps` |
| Scan carpeta SAF | `scanFolderUri(treeUri, onProgress?): List<Song>` (incluye BestiaPop; guarda abs path si se puede resolver; `importFolder` publica banner IMPORT antes del walk; toast al terminar; `identifyImportedGaps`) |
| Metadata archivo → Room | `AudioFileMetadata.fromPath` / `toSong` (`identity` + genre/year/lyrics; jaudiotagger → MMR; filename hints si Unknown / título basura, **no** pisan ID3 real; `parseCdTrackNumber` de tags CD_TRACK/DISC) |
| Upload WiFi → DB | `saveUploadedSong` (`AudioPersistRef`; merge por matchKey); VM observa `DONE` → `identifyImportedGaps` (`fillGapsOnly`, no `force`) |
| Canonicalizar URIs | `MusicRepository.migrateCanonicalAudioUris` (one-shot `canonical_audio_uris_migrated`; `getIdentitySongs` + `MusicDao.updateSongUri`) |
| Releer tags embebidos | `MusicRepository.migrateEmbeddedFileTags` (one-shot `embedded_file_tags_migrated_v4`; Unknown/generic → `fillSongGapsFromFileTags` + `splitUsingKnownArtists`; leftover → `identifyImportedGaps`) |
...
| Enriquecer meta/letras | `enhanceSongMetadataAndLyrics` lee `getSongById` antes de decidir huecos (el listado slim no trae lyrics); portada/letras/duración; **no** artist/álbum; **no** `fetchLyrics`/`fetchAlbumArtUrl` si `isPlaceholderArtist` |
...
| Aplicar candidato | `applySongIdentities` (`IdentifyApplyRequest`) en una transacción `updateSongIdentity` (no toca `lyrics`); one-shot `applySongIdentity` delega. Merge `candidate.identity.mergePreferring(entity)` + clean title / fallback album / `pickPersistedAlbumName` (`studioAlbumKeysByArtist` una vez por batch; `bucketArtists` cacheado por álbum). Tags ID3 en `tagWriteScope` después de devolver ids. |
| Identificar lote | VM `identifySongs` → `ProcessIdentifyRuntime.submit` (hasta `IDENTIFY_PARALLEL=3`; `PERSIST_EVERY=8`; snapshot identidad `getIdentitySongs` / `getSongsByIds`; HIGH auto **sin título** salvo `fillGapsOnly` + título basura; skip `pendingSongIds`; append si ya corre; `fillGapsOnlySongIds`; fan-out scoped tras HIGH) |
| Identificar una | VM `identifySongForReview` (menú ⋮; si pending → abre ítem, si no setup `force=true`); UI `IdentifyReviewScreen` |
...
| Review acciones | `applySelectedIdentifyCandidate` / `skipIdentifyReviewItem` / `searchIdentifyCandidates` / `loadMoreIdentifyCandidates` / `dismissIdentifyReview` (oculta) / `showIdentifyReview` / `applyRemainingIdentifySuggestions` (MEDIUM) / `skipAllIdentifyReview` / `applyIdentifyAlbumGroup` / `startIdentifyItemReview` / `returnIdentifyReviewOverview` / `setIdentifyReviewApplyFields`; “Buscar otro” + filtros artista/álbum/año (`IdentifySearchFilters` / `IdentifyCatalogQuery` / `identifySearchDraft`); “Mostrar más” revela pool local o pagina catálogo (`appendCandidates`, sin reordenar visibles). Aplicar exige `applyFields.hasAny` y `!isApplying`. Usar/grupo/restantes publican leftover y sueltan el mutex **antes** de Room (`beginOptimisticIdentifyApply` / `confirmOptimisticIdentifyApply`; `leftoverIdentifyReview` no pisa chips de sesión; `isApplying` no bloquea el resto de la cola). Eco de `IdentifyReviewStore` (`identifyPersistEcho`): overlay abierto con los mismos IDs **no** re-scan Room ni toma el mutex durante IO; extras del lote solo `getSongsByIds`; hydrate IO fuera del mutex. Confirm restaura leftover si Room tira. DataStore: leftover no-vacío vía debounce; `persistIdentifyReviewNow` al vaciar/restaurar. `applyKnownAlbumFanOut` en launch aparte (biblioteca, catálogo solo unmatched). Tags ID3 no bloquean el apply. `LibraryProjectionState` no emite lista nueva mientras el overlay está abierto. `IdentifyReviewItem.song` va sin lyrics (`Song.forIdentifyReview`).
| Review persist | `IdentifyReviewStore` (`appendProposals` / `mergeUiRemaining` / `removeSongIds` / `pendingSongIds`) + `IdentifyReviewCodec` (sin `audioUrl` CDN); hydrate `identifyReviewFromPersisted` + `attachKnownAlbumMatches` (huérfanos fuera); prune al borrar canciones |
| Lote en curso | `IdentifyWorkStore` + `IdentifyWorkCodec` (remaining IDs, force, fields, `fillGapsOnlySongIds`, contadores); resume `ProcessIdentifyRuntime.resumeInterrupted`; pausa si `!isCurrentlyOnline` |
| Lifetime identify | `IdentifyExecutionLauncher` UIDT `IdentifyJobService` API 34+ / FGS `IdentifyForegroundService` API 26–33; notif `IdentifyNotificationHelper` (`TAB_IDENTIFY_REVIEW`) |
| Identify ranking | `IdentifyRanking.score` / `rank(limit)` / `appendCandidates` / `confidence(ranked, query?)` / `hasSevereConflict` / `titleCollidesWithArtistOrAlbum` / `isGenericIdentifyTitle` / `preferBilingualTitle` / `catalogSearchText` / `isGenericAlbum` / `isPlaceholderArtist` (única fuente de artista débil; usada por filename hints + `AudioFileMetadata`) / `isPreferredProvider` (Deezer/iTunes/Catalog/ListenBrainz/MusicBrainz) / `cleanIdentityTitle` / `fieldSimilarity` / `titleFieldSimilarity` / `scriptRunSimilarity` / `compactSimilarity`; `Query.sourceArtist`/`sourceTitle`/`sourceAlbum`/`preferYear`; query refine `IdentifyCatalogQuery` / `identifySearchTexts` / `IdentifyQueryVariants`; grupos `clusterIdentifyAlbumGroups`; diffs de apply `identifyApplyChanges` / `IdentifyApplyField`; huecos import `IdentifyGaps.gapApplyFields` / `needsGapIdentify` / `isWeakIdentityTitle` (título/artista/álbum/portada; no año ni pista); álbumes conocidos `IdentifyKnownAlbums` (`knownAlbumsFromLibrary` / `matchSongToKnownAlbum` / `assignUniqueKnownAlbumMatches` / `loadKnownAlbumTracks`); fill de tags `fillSongGapsFromFileTags` |
| Progreso biblioteca | `libraryJobProgress` (`LibraryJobKind.IMPORT` \| `IDENTIFY` \| `TAG_WRITE`) + `LibraryProgressBanner` (botón «Abortar» para `IDENTIFY` con diálogo de confirmación y cancelación en `ProcessIdentifyRuntime.cancelUser`); pending `IdentifyPendingBanner` |
...
| Auto al guardar | `MusicRepository.maybeWriteTags` tras `applySongIdentities` / `updateSongMetadata` / album propagate+merge / download / enhance (si cambia artwork) |
...
| Omitir existentes | `GET /existing-files` = Room basename ∪ `MusicFileStore.listManagedNames`, **saneados con `UploadNameSanitizer.matchingBasenames`** (`sanitize` unicode + `asciiLegacy`) igual que el dashboard y que el nombre con el que se guarda |
...
| Identify al importar | Tags embebidos (ID3) → Room → `identifyImportedGaps` (solo huecos; `fillGapsOnly`); conflictos en cola persistida compartida (omite ya pending) |
...
- Googlevideo rechaza con 403 requests iniciales sin `Range` acotado **y** un GET que cubre todo el `clen`. L1 `GoogleVideoRange.remainingLength` / `httpRangeHeader` / `nextChunkRange` (`data/network/GoogleVideoRange.kt`). Playback: `MusicService.boundGoogleVideoRequest` acota `DataSpec.length` desconocido con `clen`. Descarga: `downloadAndSaveOnlineTrack` pide chunks cerrados (`DEFAULT_CHUNK_BYTES`) y nunca `bytes=0-{clen-1}` en un solo GET.
...
| HTTP ExoPlayer | `StreamPlaybackTag` + `setStreamPlaybackTag` / `streamPlaybackTag()` (`RequestMetadata.extras`) + `MusicService` `UserAgentMediaSourceFactory` / `boundGoogleVideoRequest` (UA + `GoogleVideoRange` `clen`→`DataSpec.length`; placeholder Remote `Uri.EMPTY` queda IDLE, no `SilenceMediaSource`) |
...
**Invariantes:**
- `MusicPlayerViewModel.onAppForeground` marca UI attached; `attachPlaybackUi` (post primer frame en `MainActivity.onStart`) llama `PlaybackRuntime.attachUi`. `onCleared` llama `detachUi`. El ViewModel no guarda `MediaController`, cola ni estado de resolución.
...
- Sin sesión viva: `PlaybackHydration.hydrateQueue` rematchea Local por id/uri y conserva Remotes identity+mbid+query/`videoId` sin CDN. Una cola solo-Remote se hidrata aun con biblioteca vacía; si current local se borró, avanza y pone posición 0. El seed idle no espera el `allSongsFlow` completo: `songsForHydration` carga Local por ids (`loadSongsByIds`) y `maybeSeedIdlePlayer` corre en cuanto hay settings + controller.
...
| Capacidad | Entry point |
|-----------|-------------|
| Lease UI + mensajes | `PlaybackRuntime.attachUi` / `detachUi` / `events`; wiring en `MusicPlayerViewModel.attachPlaybackUi` / `onCleared` |
