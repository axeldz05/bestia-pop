...
- Álbumes **ya conocidos** (biblioteca no genérica) alimentan match 1:1 por título/duración/pista (`IdentifyKnownAlbums` / `assignUniqueKnownAlbumMatches`). En `proposeSongIdentity` un hit **global único** es HIGH sin catálogo. Tras Usar/HIGH, fan-out **scoped** al tracklist Room+Deezer/iTunes (`loadKnownAlbumTracks`) aplica siblings únicos de la cola; ambiguos quedan. En revisión el fan-out corre **después** de publicar leftover. Al hidratar revisión, hits globales se promocionan a MEDIUM para el overview (`attachKnownAlbumMatches`).
