...
UI: `LibraryScreen` (`NestedLibraryBrowse` para detalle álbum/artista/género), `LibraryFilterChipRow`, `LibraryBrowseSortSheet`, `LibrarySongList` (`FastScrollContainer`), `LibraryAlbumBrowseList` (`TauonAlbumHeader` + `FastScrollContainer`), `LibraryAggregateListItem` + `LibraryArtistList` / `LibraryGenreList` (`FastScrollContainer`), `LibrarySettingsScreen`.
...
Headers de grupo: `LibraryListItem.AlbumHeader` lleva `albumName` (clave), `displayName` (override), `songIds` y `sortHint` — `TauonAlbumHeader(title = …)` muestra el display name, así que Canciones y Álbumes no divergen tras renombrar sin propagar. Play/select del header resuelve ids con `songsInOrder`.
