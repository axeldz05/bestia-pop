...
Estado: `ui/state/LibraryProjectionState.kt`, `UiNavigationState.kt`, `LibraryBrowseFilter.kt`, `LibraryListItem.kt` (`LibraryListModel` / `LibraryAlbumSegment`) y `LibraryUiState.kt` (solo `LibraryViewMode`). Prefs: `LibraryDisplaySettings` (`sortOptionName` / `sortDirectionName`), `FastScrollSettings` (`FastScrollSide.RIGHT` / `LEFT`) + `UiNavSnapshot.browseFilterName` / `LibraryUiPreferencesCodec` (legacy `library_tab` 0/1/2 → SONGS/ALBUMS/ARTISTS).
