...
**Invariante cola:** todas las descargas online se registran en `ProcessDownloadCoordinator.downloads` (`QUEUED` → `DOWNLOADING` → `SUCCESS`/`ERROR`); éxito se retiene hasta dismiss. `ProcessDownloadRuntime` posee jobs fuera del VM; coordinator reclama plain/`batch:` y comparte `Semaphore(3)`. Cada row persiste destinos, lookup, batch/policy y stages; `updateProgress` coalesce snapshots de porcentaje, mientras altas/cancelaciones/conflictos/terminales siguen con persist inmediata y `flushDurably` ocurre antes del lifetime/bytes y tras commit storage. Tras kill o stop de Job, active → ERROR interrumpido; `resumeInterrupted` reencola tanto en cold start como en `MainActivity.onStart` warm salvo salida `REASON_USER_REQUESTED`. Explícitas: UIDT API34+ / FGS dataSync API26–33; Guardar al escuchar: constrained job. `BackgroundExecutionStatus` separa restricción Android de exención Doze y `backgroundRestrictionGuidance(manufacturer)` elige el copy (Motorola/Lenovo Recientes, Samsung never-sleep, Xiaomi autostart, default Unrestricted). Si el dispositivo resuelve `unisoc.intent.action.POWER_BACKGROUND_CLEAN`, `MainScreen` ofrece una vez ir a Batería (`lock_screen_battery_save`) vía `BackgroundExecutionProbe.openOemScreenOffCleanupSettings`.
...
| Settings UI | `PlaybackSettingsScreen` vía `SettingsScreen` sección Reproducción; Batería muestra por separado `blocksBackgroundPlayback` vs Doze; copy vía `backgroundRestrictionGuidance`; botón OEM `openOemScreenOffCleanupSettings` si el intent resuelve |
