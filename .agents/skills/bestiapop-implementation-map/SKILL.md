...
| Pantalla | Archivo |
|----------|---------|
| Shell + bottom nav | `ui/screens/MainScreen.kt` (`ConfirmMergeAlbumsDialog` vía `pendingAlbumMerge`; `AppUpdateDialogs` + check al abrir; banner `blocksBackgroundPlayback` + `restrictionGuidance`; diálogo OEM solo si `oemScreenOffCleanupActive`) |
...
| Playlists | `ui/screens/PlaylistsScreen.kt` (`searchQuery` filter; integrado como chip `PLAYLISTS` en `LibraryScreen`; `BackHandler`: CF → LB → local detail; play/shuffle vía `LabeledPlayShuffleButtons` + `playCollection`/`shuffleCollection`; `MatchedTrackLazyColumn`; `matchedStreamCountLabel`; `rememberSongQueueActions`) |
