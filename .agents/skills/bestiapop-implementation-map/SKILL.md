...
| Pantalla | Archivo |
|----------|---------|
| Shell + bottom nav | `ui/screens/MainScreen.kt` (`SaveableStateHolder` preserva estado/scroll de pestañas; `ConfirmMergeAlbumsDialog` vía `pendingAlbumMerge`; `AppUpdateDialogs` + check al abrir; banner `blocksBackgroundPlayback` + `restrictionGuidance`; diálogo OEM solo si `oemScreenOffCleanupActive`) |
| System back (exit doble + orquestación) | `MainScreen` `BackHandler` + `SnackbarHost`; nested en screens abajo; identify review overlay |
| Biblioteca | `ui/screens/LibraryScreen.kt` (`BackHandler`: multi-select / addition / album-artist-genre / search; Tune → `LibraryBrowseSortSheet`; chips `LibraryFilterChipRow`; `PlayShuffleIconPair`; `LibraryBrowsePane` + `LibrarySongListHost` + `rememberSongActionDialogs`) |
...
| Lista canciones / álbumes / artistas / géneros | `ui/screens/library/LibrarySongList.kt` (`currentSongIdFlow` + highlight `derivedStateOf`; `filterCollapsedAlbumSongs` / `AlbumHeader.matchesCollapsed` / `albumHeaderSelectionState`; Coil async artwork + `rememberLazyListState`); `ui/components/FastScrollScrubber.kt` (`FastScrollContainer`, `FastScrollDefaults.dedicatedStartGutterWidth` / `dedicatedEndGutterWidth` con margen dedicado lateral según la posición activa, `FastScrollScrubber`, `FastScrollRail`, `FastScrollIndicatorBubble`, `FastScrollSections` adaptativo Niagara con tick háptico, orden natural A-Z / números / símbolos y thumb de posición); `LibrarySongListHost.kt` (`LibrarySongListActions` `@Immutable`, `rememberSongActionDialogs` / `SongActionDialogsHost`, `AlbumEditDialogsHost`), `LibraryFilterChipRow.kt`, `LibraryBrowseSortSheet.kt` (`libraryOrderSummary`, `SortOption.sortLabel`, `sortDirectionLabel`, `albumHeadersActive`), `LibraryAlbumBrowseList.kt` (`AlbumHeader` + `FastScrollContainer`), `AlbumEditCoverMenuItems.kt`, `LibraryAggregateListItem.kt` (fila artista/género), `LibraryArtistList.kt` (`FastScrollContainer`), `LibraryGenreList.kt` (`FastScrollContainer`), `LibraryDialogs.kt` (`EditSongMetadataDialog` Nº de pista, `EditAlbumMetadataDialog`, `ConfirmMergeAlbumsDialog`, `SetAlbumArtworkDialog`, `AddToPlaylistDialog` con soporte multi-selección, creación inline y abrir playlist), `ui/components/PlaylistFormDialog.kt` (`PlaylistFormDialog` con acciones Crear y "Crear y entrar"), `EditLyricsDialog.kt` (tabs Texto/Sincronizar + playback), `LibraryProgressBanner.kt` + `IdentifyPendingBanner` |
| Descubrir | `ui/screens/discover/DiscoverScreen.kt` (`DiscoverTopSearchBar`, `DiscoverCategoryChipsRow`, `DiscoverRecentSearchesView`, `DiscoverHomeFeedView`, `DiscoverSearchResultsView`, `DiscoverCollectionDetailView`, `DiscoverCollectionActions`, `DiscoverAdvancedFiltersPanel`, `DiscoverTrackListItem`, `ItemLibraryStatusIcon`), `DiscoverLbPlaylists.kt` (`DiscoverMatchedTrackActions`, `MatchedPlaylistContent`, `CfRecommendationsDetailScreen`, `LbPlaylistDetailScreen`, `DiscoverPlaylistDetailHost`) |
| Playlists | `ui/screens/PlaylistsScreen.kt` (`searchQuery` filter; integrado como chip `PLAYLISTS` en `LibraryScreen`; `BackHandler`: CF → LB → local detail; filas con `PlaylistHeader` estilo `AlbumHeader`, botones rápidos de reproducción y aleatorio, menú ⋮ con edición inline `PlaylistFormDialog`, eliminación y encolar; tap en cabecera abre detalle; play/shuffle vía `LabeledPlayShuffleButtons` + `playCollection`/`shuffleCollection`; `MatchedTrackLazyColumn`; `matchedStreamCountLabel`; `rememberSongQueueActions`; soporte para canciones duplicadas, modo "Mover canciones" para reordenar interactivamente vía drag handle `Icons.Default.DragHandle` + persistencia en Room vía `MusicDao.reorderPlaylistSongs`; creación vía `PlaylistFormDialog` con opción de entrar directo) |
...
| Ajustes / Biblioteca | `ui/screens/LibrarySettingsScreen.kt` (`SettingsScreen` sección `Library`; configuración de qué blobs/categorías mostrar en biblioteca y reordenamiento interactivo vertical drag & drop vía `LibraryBlobsSettings` + `rememberVerticalReorderDrag` y asignación del principal; scroll rápido vertical estilo Niagara on/off + borde derecho/izquierdo para zurdos vía `FastScrollSettings`) |
...
| `CollectionUtils` | `domain/util/CollectionUtils.kt` | Algoritmos reutilizables de colecciones: intercalado equitativo (`interleaveEquitable`) y scoring/ranking de artistas locales (`calculateTopLocalArtists`) |
...
| Identify gaps | `domain/util/IdentifyGaps.kt` | `isWeakIdentityTitle` / `gapApplyFields` / `needsGapIdentify` / `songHasGapsForFields` (título/artista/álbum/portada/pista/año según `applyFields`) |
...
| Modelos dominio UI | `data/model/Models.kt`: `OnlineCatalogTrack` / candidates/catalog; `ActiveDownload` (`TrackMeta`, transitions `asDownloading`/`asConflict`/`asSuccess`/`asError`, `restoredConflictPolicy`); `DownloadLane` + `ActiveDownloadSource.lane`; `CandidateDownloadState.isInFlight/isFailed`; único `DownloadPlaylistDestination` + `resolveDownloadPlaylistDestinations`; playlists/WiFi/identify models. `Song.kt` (`@Immutable`) / `AlbumOverride.kt` / `SongPlayStat.kt` = Room planos |
...
| Room DB | `data/db/AppDatabase.kt` (v13; `MIGRATION_9_10` `song_play_stats`, `MIGRATION_10_11` índices individuales, `MIGRATION_11_12` title, `MIGRATION_12_13` compuesto `artist_album`) |
| DAO | `data/db/MusicDao.kt` (`getAllSongsFlow` identity-slim sin `lyrics` ni `ORDER BY`, `lastPlayedAt=0`; `getPlayStatsFlow` / `getPlayStat` / `deletePlayStatsForSongs`; `getSongById` / `getAllSongs` fila completa; `insertSong` / `insertSongs` = `OnConflictStrategy.IGNORE`, `getSongByUri` / `getSongsByUris`, `getIdentitySongs` / `getIdentitySongsByIds`, `updateSongIdentity`, `deletePlaylistRefsForSongs`, `getPlaylistIdsForSong`, `getCoPlaylistSongIds`, `updateLastPlayedAt` → stats) |
...
| YouTube search + stream | `data/network/YouTubeExtractor.kt` (`YouTubeStreamResult(identity, videoId, audioUrl, userAgent)` : `TrackMeta` + invoke L2, `extractYouTubeId`, `searchYouTube`, `parseSearchContents`, `audioPreferenceScore`, `rankByAudioPreference`, `resolveYouTubeQueryOrId`, `AUDIO_CLIENTS` TVHTML5 5.x / VISIONOS / ANDROID / ANDROID_MUSIC / TVHTML5 7.x; no ANDROID_VR) |
...
| Library initial scan + UI prefs | `data/preferences/LibraryPreferencesRepository.kt` (`isInitialScanCompleted`, `displaySettingsFlow`, `navSnapshotFlow`, `setSortOptionName` / `setSortDirectionName` / `setViewModeName` / `setNavSnapshot`, one-shots `canonical_audio_uris_migrated` / `device_date_added_migrated` / `embedded_file_tags_migrated_v4`); codec `LibraryUiPreferences.kt` (`LibraryUiPreferencesCodec`, `LibraryStackLookups.fromSongs`, `UiNavSnapshot.browseFilterName` + legacy `library_tab`, `LibraryDisplaySettings` sort+direction, `sanitizeBrowseFilterName` / `sanitizeSortDirectionName`) |
...
| ListenBrainz API | `data/network/ListenBrainzClient.kt` (`submitListens`, createdfor, playlist, `lookupRecordingMetadata`, `fetchLbRadioArtist`, `fetchRecordingMetadata`, `fetchCfRecordingRecommendations`, `fetchUserTopArtists`, `fetchUserTopReleases`, `fetchUserTopRecordings`, `fetchUserRecentListens`) |
...
| Storage / transfer helpers | `data/util/MusicFileStore.kt` (`readableFile` / `writableFile`), `AudioPersistRef.kt`, `StorageUtils.kt` (PendingWrite/publish/list/delete), `TransferIo.kt` (`copyTransferToFile`: L1 compartido música+APK; políticas quedan arriba), `SongPathNormalizer.kt` (`toAbsolutePath` también `/tree/` root), `UploadNameSanitizer.kt` (`sanitize` / `asciiLegacy` / `matchingBasenames`), `AudioFileMetadata.fromPath` (jaudiotagger + MMR fallback, year/lyrics, `AlbumArtworkCache` opcional), `AudioTagReader.kt`, `AudioTagWriter.kt`, `SyncedLyrics.kt` (`parse` / `format` / `plainText` / `realignByText` / `stamp` / `currentLineIndex`) |
...
| Runtime de identify process-scoped | `service/ProcessIdentifyRuntime.kt` (`submit(..., fillGapsOnly)` / `resumeInterrupted` / `cancelUser` / `settle`; `IDENTIFY_PARALLEL=3` workers; `PERSIST_EVERY=8`; `getSongById` / `getSongsByIds`; HIGH auto `fields.copy(title = false)` o `gapApplyFields` si `fillGapsOnlySongIds`; fan-out scoped `loadScopedAlbumTracks` tras HIGH; pausa offline; progress IDENTIFY; review bufferizada; eventos Completed/AlreadyQueued) |
...
| Notificación playback / controllers externos | `PlaybackNotificationContractTest.kt` (foreground policy Remote IDLE + sticky resume + `WAKE_MODE_NONE` local); `BackgroundExecutionStatusTest.kt` (`blocksBackgroundPlayback` exige `isBackgroundRestricted` + AppOps IGNORE; `BestiaPopMediaLibraryCallbackTest.kt` (policy transporte-only no confiable); `LocalPlaybackServiceFunctionalTest.kt` (provider real, acciones/FGS); `LockedScreenPlaybackFunctionalTest.kt` (host-only sin appops) |
