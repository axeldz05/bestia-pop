...
| Lista canciones / álbumes / artistas / géneros | `ui/screens/library/LibrarySongList.kt` (`currentSongIdFlow` + highlight `derivedStateOf`; `filterCollapsedAlbumSongs` / `AlbumHeader.matchesCollapsed` / `albumHeaderSelectionState`; Coil async artwork + `rememberLazyListState`); `LibrarySongListHost.kt` (`LibrarySongListActions` `@Immutable`, `rememberSongActionDialogs` / `SongActionDialogsHost`, `AlbumEditDialogsHost`), `LibraryFilterChipRow.kt`, `LibraryBrowseSortSheet.kt` (`libraryOrderSummary`, `SortOption.sortLabel`, `sortDirectionLabel`, `albumHeadersActive`), `LibraryAlbumBrowseList.kt` (`TauonAlbumHeader`), `AlbumEditCoverMenuItems.kt`, `LibraryAggregateListItem.kt` (fila artista/género), `LibraryArtistList.kt`, `LibraryGenreList.kt`, `LibraryDialogs.kt` (`EditSongMetadataDialog` Nº de pista, `EditAlbumMetadataDialog`, `ConfirmMergeAlbumsDialog`, `SetAlbumArtworkDialog`), `EditLyricsDialog.kt` (tabs Texto/Sincronizar + playback), `LibraryProgressBanner.kt` + `IdentifyPendingBanner` |
...
| Artwork UI | `ui/components/ArtworkThumbnail.kt` (`rememberArtworkRequest` + `Precision.INEXACT`, `isArtworkCachedInMemory`, `ArtworkThumbnail` con `isScrollInProgress` y `allowIntermittent`, `ArtworkHero`); `ui/components/ArtworkPicker.kt` (`ArtworkPickerBlock`, `rememberImagePicker`) |
