...
Ownership process-scoped: `service/PlaybackRuntime.kt` (`create`, `attachUi`, `detachUi`, `events`; cola + `DiscoverPlaybackOrigin`, controller demand-driven, reconnect con backoff sin leer el facade desconectado, intención/slot/posición propios, callbacks externos de timeline/repeat/shuffle, generación de resolve/prefetch/recovery, posición/tracker/radio y writer serial de sesión) y `service/ProcessDownloadRuntime.kt` (jobs/manual retry/conflictos/eventos) retenidos por `BestiaPopApplication`; `service/PlaybackMediaItemCodec.kt` es la frontera Media3; `ProcessSaveWhileListeningCoordinator` conserva la semántica autosave y `ProcessDownloadCoordinator` unifica ids/permiso/cola/persistencia/destinos. Los servicios/job Android solo dan lifetime/notificación; el ViewModel es fachada UI.

| Servicio | Rol |
|----------|-----|
| `MusicService` | `MediaLibraryService` + ExoPlayer (`WAKE_MODE_NETWORK` + `WAKE_LOCK`); única notif `playback_channel`/1001 vía `PlaybackNotificationFactory` (Previous · Play/Pause · Next, intents `ACTION_MEDIA_BUTTON` al service, compact 0/1/2) y FGS `mediaPlayback` vía `promotePlaybackForeground` (`startForeground`; sin provider paralelo ni auto-promoción Media3 por `startForegroundService`; `PlaybackServiceLifetimePolicy` considera engaged `playWhenReady` + cola incluso en `STATE_IDLE` de placeholder; `onTaskRemoved` no corta ese caso) + `setSessionActivity`; `UserAgentMediaSourceFactory` lee UA de `StreamPlaybackTag` y `boundGoogleVideoRequest` acota requests googlevideo abiertos usando `clen`; `StereoBalanceAudioProcessor` + `LoudnessEnhancer` desde `PlaybackPreferencesRepository` |
