...
| Capacidad | Entry point |
|-----------|-------------|
| UI cola | `DownloadsScreen` + `ActiveDownloadRow`. **Toda fila cancelable:** `DownloadStateTrailing` muestra botón de cancelar también en QUEUED y DOWNLOADING (`DownloadInFlightActions`) y retry+dismiss en IDLE; sin eso una fila trabada no se podía sacar y el gate de encolado rechazaba todo intento futuro de ese tema. Wired en Descargas, detalle de playlist (`MatchedTrackRow.onCancelDownload`) y Now Playing |
...
- **Escribir el stream resuelto va por `applyResolvedRemote(resolved)`**, que localiza el slot por `mediaId`. El índice capturado antes de la red puede apuntar a otro tema al volver (borrado, refill de radio, reshuffle) y se sobrescribía esa otra canción. Lo usan `ensureRemoteReadyAt`, `prefetchAround`, `handlePlayerError` y `skipToQueueIndex`.
