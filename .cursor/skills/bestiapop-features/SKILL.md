...
| Vista plana vs grupos álbum | `LibraryViewMode.FLAT` / `ALBUM_GROUPS` → `setLibraryViewMode` / `toggleLibraryViewMode`; solo UI cuando chip = `SONGS`; `buildLibraryListItems` / `buildListItems(sortOption, sortDirection)` (bloques = `extractAlbums` por `songsByAlbumBucket`; cada canción en **un** grupo; LazyColumn key = `Album.groupingKey` / `AlbumHeader.groupingKey`, no el título visible); within-album `compareSongsWithinAlbum` / `sortSongsWithinAlbum` / `songsFromListItems` / `songsForAlbum` |
...
| Colapsar álbum / todos | `collapsedAlbumNames` + toggle por header (`onToggleCollapseAlbum`); `filterCollapsedAlbumSongs` oculta **todas** las filas bajo el header (`matchesCollapsed` por `albumName`/`groupingKey`, no `Song.album` crudo); expandir/colapsar todo usa nombres de header (`LibraryScreen`; vista grupos; no persistido) |
...
Herencia visual en lista: `GetLibrarySongsUseCase.execute` unifica artwork faltante desde otras canciones del mismo **bucket** de álbum (`albumGroupingKey`; **omite álbumes genéricos**: «Unknown Album» es el literal de todas las canciones sin álbum, así que heredar ahí estampaba una portada sobre temas no relacionados); `extractAlbums(songs, overrides, sortOption, sortDirection)` agrupa por esa clave (no el string crudo), aplica `AlbumOverride` y ordena agregados.
...
- Tags Unknown / rip numérico: `AudioFileMetadata.applyFilenameHints` + `parseFilenameMetadataHints` / `resolveWeakIdentityHints` / `tidyFilenamePhrase` / `identifySearchTexts`; al `proposeSongIdentity` persiste limpieza soft de **basura** (`01`→Unknown Artist, `- Title`→Title, trackNumber, path) **sin** pisar un título ID3 real. Filename hints siguen armando la **consulta**. `Artist_Title` solo parte en un `_` (`Radiohead_Creep`); varios `_` quedan frase buscable (`The_Doors_Roadhouse_Blues`). Huecos del sanitizer (`__` CJK/tildes, `It_s`→`It's`, `16-44.1 kHz`) se colapsan; identify busca cola distintiva / agujero pegado y, si Deezer no alcanza, iTunes+YouTube (`searchIdentifyFallbacks`).
- Identificar online (manual, multi-select, ⋮ o WiFi): lote **process-scoped** (`ProcessIdentifyRuntime.submit`; no `viewModelScope`). UIDT API 34+ / FGS `dataSync` API 26–33 mientras hay remaining. Fase 1 lookup + score (`IdentifyRanking`); tags actuales de la canción son **fuente predominante** (`sourceArtist`/`sourceTitle`/`sourceAlbum`); auto-aplica solo **HIGH** y **nunca el título** (`applyFields.copy(title = false)` / `identifySongMetadata`); en revisión el usuario sí puede aceptar otro. Conflicto grave (artista/álbum/título distinto, versión, YouTube) o título igual a artista/álbum (`titleCollidesWithArtistOrAlbum`) → nunca HIGH (busca `catalogSearchText` artista+álbum y rankea por duración). Score también compara artista+título juntos (`fieldSimilarity` / `compactSimilarity`) para no penalizar el split `The`/`Doors Roadhouse Blues` cuando el catálogo tiene The Doors — Roadhouse Blues. Si LB `enabled` + token y confianza ≠ HIGH (sin `customQuery`/filtros ni título=artista), enriquece con `lookupRecordingMetadata` → `fetchRecordingMetadata` → `toListenBrainzCatalogTrack` y re-rank (`IdentifyProposal.usedListenBrainz`). MEDIUM/LOW/NONE se **anexan por canción** a cola persistida `IdentifyReviewStore` (DataStore; cold start hidrata, **no** abre overlay). Remaining IDs en `IdentifyWorkStore`; resume en `onAppForeground` salvo `REASON_USER_REQUESTED`. UI `IdentifyReviewScreen`: overview por álbum sugerido (`clusterIdentifyAlbumGroups`, solo MEDIUM + álbum no genérico, size ≥ 2) o cola canción a canción (Usar / Omitir / Buscar otro + filtros artista·álbum·año / Mostrar más candidatos / Aplicar automático a restantes = solo MEDIUM con suggested / Omitir todas / Aplicar grupo). Header muestra chips editables de `IdentifyApplyFields` (`IdentifyApplyField` / `setIdentifyReviewApplyFields`; persistidos con la cola). Cada candidato lista diffs (`identifyApplyChanges` / `formatIdentifyApplyChanges`) solo de campos tildados que cambiarían. “Buscar otro” siembra **título** en el draft y artista/álbum/año en filtros (`identifySearchDraft` / `identifySearchFilterArtist`); no concatena artista+título. «Tu archivo» muestra año/pista y aviso si el título colisiona. Cerrar oculta overlay y **conserva** la cola (`pendingCount`); banner biblioteca + botón WiFi + notif retoman. Re-identificar lote/WiFi **omite** songIds ya pendientes (cero red); ⋮ sobre pending abre ese ítem. Un segundo pedido **agrega** IDs al lote en curso. Overlay ya abierto: anexar propuestas (`mergeIncomingReviewItems`) **no** reinicia buscador, filtros, selección ni candidatos del ítem actual; `showIdentifyReview` no-op si ya visible. Progreso `libraryJobProgress` (merge runtime IDENTIFY + import/tag-write local) + toast resumen si hay UI; overlay al completar solo si `showReview` y UI attached. Telemetría lote (sin PII): `CrashReporter.setKey`/`log` keys `identify_high`/`identify_medium`/`identify_low`/`identify_none`/`identify_skipped`/`identify_lb_hits`.
...
| Metadata archivo → Room | `AudioFileMetadata.fromPath` / `toSong` (`identity` + genre; filename hints si Unknown / título basura, **no** pisan ID3 real; `parseCdTrackNumber` de tags CD_TRACK/DISC) |
...
| Aplicar candidato | `applySongIdentity` → `candidate.identity.mergePreferring(entity)` (+ clean title / fallback album / `pickPersistedAlbumName`) → `Song.withIdentity` → `IdentifyResult` |
| Identificar lote | VM `identifySongs` → `ProcessIdentifyRuntime.submit` (hasta `IDENTIFY_PARALLEL=3` a la vez; HIGH auto **sin título** salvo `fillGapsOnly` + título basura; skip `pendingSongIds`; append si ya corre; `fillGapsOnlySongIds`; fan-out scoped tras HIGH) |
...
| Ranking | `IdentifyRanking.score` / `rank(limit)` / `appendCandidates` / `confidence(ranked, query?)` / `hasSevereConflict` / `titleCollidesWithArtistOrAlbum` / `catalogSearchText` / `isGenericAlbum` / `isPlaceholderArtist` (única fuente de artista débil; usada por filename hints + `AudioFileMetadata`) / `isPreferredProvider` (Deezer/iTunes/Catalog/ListenBrainz) / `cleanIdentityTitle` / `fieldSimilarity` / `compactSimilarity`; `Query.sourceArtist`/`sourceTitle`/`sourceAlbum`/`preferYear`; query refine `IdentifyCatalogQuery` / `identifySearchTexts`; grupos `clusterIdentifyAlbumGroups`; diffs de apply `identifyApplyChanges` / `IdentifyApplyField`; huecos import `IdentifyGaps.gapApplyFields` / `needsGapIdentify` / `isWeakIdentityTitle` (título/artista/álbum/portada; no año ni pista); álbumes conocidos `IdentifyKnownAlbums` (`knownAlbumsFromLibrary` / `matchSongToKnownAlbum` / `assignUniqueKnownAlbumMatches` / `loadKnownAlbumTracks`) |
