...
| Concern | Archivo |
|---------|---------|
| Repo impl | `data/repository/MusicRepository.kt` (`scanMediaStore`, `resyncAppManagedMusic`, `scanFolderUri`, `proposeSongIdentity` (+ `listenBrainzToken`, `IdentifySearchFilters` / `IdentifyCatalogQuery` / `identifySearchTexts`, soft `persistWeakIdentityCleanup`, edge LB via `fetchListenBrainzIdentifyTrack`, append page, `searchIdentifyFallbacks` si LOW/NONE), `applySongIdentity`, `identifySongMetadata`, `maybeWriteTags` / `syncTagsToFiles`, `migrateCanonicalAudioUris`) |
...
| Identify work persist | `data/preferences/IdentifyWorkStore.kt` (`IdentifyWorkCodec`, `IdentifyWorkSnapshot` remaining IDs) |
...
| Storage / transfer helpers | `data/util/MusicFileStore.kt`, `AudioPersistRef.kt`, `StorageUtils.kt` (PendingWrite/publish/list/delete), `TransferIo.kt` (`copyTransferToFile`: L1 compartido música+APK; políticas quedan arriba), `SongPathNormalizer.kt`, `UploadNameSanitizer.kt`, `AudioFileMetadata.fromPath` (un retriever para tags+embedded art, callback `persistEmbeddedArtwork`), `AudioTagWriter.kt`, `SyncedLyrics.kt` (`parse` / `format` / `plainText` / `realignByText` / `stamp` / `currentLineIndex`) |
...
| Runtime de identify process-scoped | `service/ProcessIdentifyRuntime.kt` (`submit` / `resumeInterrupted` / `settle`; progress IDENTIFY; review por canción; eventos Completed/AlreadyQueued) |
