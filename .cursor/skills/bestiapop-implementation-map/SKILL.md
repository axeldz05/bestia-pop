...
| Notificación playback / controllers externos | `PlaybackNotificationContractTest.kt` (foreground policy Remote IDLE + sticky resume + `WAKE_MODE_NONE` local); `BackgroundExecutionStatusTest.kt` (`blocksBackgroundPlayback` solo AppOps IGNORE + `oemScreenOffCleanupActive` + `backgroundRestrictionGuidance` por OEM); `BestiaPopMediaLibraryCallbackTest.kt` (policy transporte-only no confiable); `LocalPlaybackServiceFunctionalTest.kt` (provider real, acciones/FGS); `LockedScreenPlaybackFunctionalTest.kt` (host-only sin appops) |
