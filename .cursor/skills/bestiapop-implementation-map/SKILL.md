...
| Stream resolve + cache TTL | `data/stream/StreamResolver.kt` (`resolve` playback cache; `resolveQuery(forceRefresh)` download; `invalidate(item)` suspend borra claves `id:` + `q:` bajo el mutex). Ojo: hay **dos** instancias en runtime, una por `MusicRepository` (`MusicPlayerViewModel` y `WebServerService`); la del servidor no resuelve streams |
