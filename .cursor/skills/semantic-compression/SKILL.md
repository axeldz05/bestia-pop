...
* **Wrap, don’t copy**: `IdentifyCandidate(track, score, reasons)`, `Remote(identity, mbid, stream)`, `AudioFileMetadata(identity, genre)`, `LbPlaylistTrack` / `LbRecordingMetadata` / `MatchedRemoteTrack` / `PlaylistPendingTrack` (`identity` + mbid/score/playlist extras).
