...
- Changelog user-facing → `CHANGELOG.pending.md` (gitignored). Tras features/fixes visibles, anotar ahí (skill `bestiapop-release-changelog`). Formato estricto: `- **Área / Feature:** beneficio o acción directa` en 1-2 líneas máximo. Agrupar cambios relacionados bajo la misma feature, sin párrafos largos ni prosa excesiva. No anotar refactors ni detalle interno
...
- **Verificación previa de preexistencia (prohibido duplicar utilidades):** Antes de crear cualquier función auxiliar, helper o parser (ej. normalización de URLs, portadas, transformaciones de cadenas o cálculos de catálogo), buscar exhaustivamente en la clase actual y en el proyecto (`grep_search`) si ya existe una función que resuelva esa tarea. Reusar o adaptar la función existente en lugar de crear una paralela.
- **Sustitución y adopción global inmediata:** Al refactorizar, crear o generalizar una función canónica (ej. `Iterable<TrackMeta>.firstArtworkUri()`, `fetchTrackArtwork(track: TrackMeta)`), realizar una búsqueda en todo el repositorio para reemplazar **todas** las implementaciones locales repetidas o fragmentadas existentes por la función canónica. No dejar variantes ad-hoc dispersas en la base de código.
- **Auditoría y purga de código muerto:** Al terminar una refactorización o sustitución de funciones, verificar si la función anterior, sobrecargas intermedias o adaptadores quedaron sin invocadores en el proyecto. Si no tienen uso, eliminarlos de inmediato para evitar código zombie.
