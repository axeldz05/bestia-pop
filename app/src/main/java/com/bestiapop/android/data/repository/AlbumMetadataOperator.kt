...
internal class AlbumMetadataOperator(
    ...
    suspend fun setAlbumArtwork(
        ...
        val override =
            existing?.copy(artworkUri = artworkUri)
    ...
    suspend fun mergeAlbumInto(
        ...
        val override =
            musicDao.getAlbumOverride(canonicalTarget)
                ?: musicDao.getAlbumOverride(targetAlbumKey)

        val safeArtist =
            override?.artist?.takeIf { it.isNotBlank() }
        ...
        val artwork =
            override?.artworkUri?.takeIf { it.isNotBlank() }
                ?: targetSongs.firstArtworkUri()
